...
$sql = "insert into evaluacion (fecha_evaluacion, comentario_empleado, 
meta_proxima, desempenio, emp_clave) values ('".$fecha_evaluacion."','".$comentario_empleado."','".$meta_proxima. "','".$desempenio_general."','".$emp_clave."')";
...
if(!$resultado) {
    echo "<script> alert('Error, los datos de evaluacion no se ingresaron correctamente');
        window.location='evaluacion_view.php'
    </script>;";
} else {
...
$sqlIdEvaluacion = "select max(id_evaluacion) from evaluacion"; // Esto regresa la ultima evaluación
$result = false;
$result = mysqli_query($link, $sqlIdEvaluacion);
$id_evaluacion = mysqli_fetch_assoc($result);
...
$sqlRespuesta = 
    ...
    ('3', 'c','".$id_evaluacion."','".$respuesta3a."','".$comentario3a."'),
    ('3', 'b','".$id_evaluacion."','".$respuesta3b."','".$comentario3b."'),
    ('3', 'a','".$id_evaluacion."','".$respuesta3c."','".$comentario3c."'),
...
/* 
UPDATE `evaluacion` SET `desempenio`= (select avg(puntuacion) 
from respuesta where id_evaluacion = $el id prry ) WHERE id_evaluacion = $el id prry x 2
